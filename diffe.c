/* Diffe-Hellman key exchange proof of concept by Grif Hughes - ghughes@smu.edu */

#include <gmp.h>
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
#include <time.h>

static const char *prime = "";

int 
main(void)
{
        /* seed */
        srand(time(NULL));
        
        /* two secret values */
        unsigned int as = abs(rand());
        unsigned int bs = abs(rand());

        /* init contexts */
        mpz_t P, G, a, b, A, B, gen_one, gen_two;
        mpz_init(P); mpz_init(G); 
        mpz_init(a); mpz_init(b);
        mpz_init(A); mpz_init(B);
        mpz_init(gen_one); mpz_init(gen_two);
        
        mpz_set_str(P, prime, 10);
        mpz_set_ui(G, 2);
        mpz_set_ui(a, as);
        mpz_set_ui(b, bs);

        /* compute A and B */
        mpz_powm_sec(A, G, a, P);
        mpz_powm_sec(B, G, b, P);

        /* compute shared secret */
        mpz_powm_sec(gen_one, A, b, P);
        mpz_powm_sec(gen_two, B, a, P);

        /* allocate buffer to be passed to AES */
        char *out = malloc(mpz_sizeinbase(gen_one, 10) + 2);
        mpz_get_str(out, 10, gen_one);

        /* validate and print key */
        if(mpz_cmp(gen_one, gen_two) == 0)
                printf("secret key = %s\n", out);

        /* free allocs */
        mpz_clears(P, G, a, b, A, B, gen_one, gen_two, NULL);
        free(out);
}
